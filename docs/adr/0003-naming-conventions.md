...
1. **Nombres de archivos redundantes**: En un monorepo, los archivos ya están asilados lógicamente por carpeta. Por ejemplo: `packages/vanilla/button/src/headless-button.ts`. A esto se le conoce como *Name Stuttering* (Tartamudeo de Nombres).
...
## Decisión
Hemos estandarizado agresivamente las convenciones de *naming*:
...
2. **Prefijo `hp-` oficial**: Todos los web components creados deberán registrar su etiqueta HTML *prefixed* con `hp-` (apócope de Headless Primitives). Ej. `<hp-button>`. 
